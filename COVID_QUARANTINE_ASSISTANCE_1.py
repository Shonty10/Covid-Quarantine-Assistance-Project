...
def login():
    ...
    global login_isclicked
    login_isclicked=True
    #login_isclicked_=[]
    #login_isclicked_.append(login_isclicked)
    #global login_isclicked_
    #print(login_isclicked_)
...
def save():
    ...
    if h14!="" and h15!="" and h16!="" and h17!="":
        if healthvalid()==True:
            ...
            ##columns
            fields=["Day","Dry Cough", "Fever", "Chest Pain", "Difficulty in breathing", "Sore throat", "Body Pains and Aches", "Diarrhoea", "Headache", "Loss of taste or smell", "Tiredness", "Conjuctivits", "Rash or skin discolouration", "Loss of speech or locomotion","Temperature", "Weight", "Blood Pressure", "Heart Rate", "Health Score"]
            rows=[]
            for i in l:
                rows.append(i)
            #print(rows)
            ...
            def clicked():
                if (len(stats_rows)==4 or len(stats_rows)==8 or (len(stats_rows)==12 and avg_d3>35) or len(stats_rows)==16):
                    l=["Thank you for entering your data!","Please come back tomorrow.","You may close the application."]
                    messagebox.showinfo('Covid Quarantine Assisstance',"\n".join(l))
                    root.destroy()
                elif (len(stats_rows)>20):
                    l1=["You have already entered your helth stats for 5 consecutive days and the program has been terminated for your account.", "Please login from a different ID or create a new account.", "Thank you. We hope you liked using COVID Quarantine Assistance"]
                    messagebox.showinfo('Covid Quarantine Assisstance',"\n".join(l1))
                    root.destroy()
