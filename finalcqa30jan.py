...
def save():
    ...
    if h14!="" and h15!="" and h16!="" and h17!="":
        if healthvalid()==True:
            ...
            def clicked():
                if (len(stats_rows)==4 or len(stats_rows)==8 or (len(stats_rows)==12 and avg_d3>45) or len(stats_rows)==16):
                    l=["Thank you for entering your data!","Please come back tomorrow.","You may close the application."]
                    messagebox.showinfo('Covid Quarantine Assisstance',"\n".join(l))
                    root.destroy()
                    ...
                    messagebox.showinfo('Covid Quarantine Assisstance',"\n".join(l1))
                    root.destroy()
                elif (len(stats_rows)>=12 and avg_d3<=45):
                    l2=["The final result has already been declared and the program has been terminated for your account.", "Check your e-mail to view a copy of your final health report or please login from a different ID/create a new account.", "Thank you. We hope you liked using COVID Quarantine Assistance"]
                    root.destroy()
